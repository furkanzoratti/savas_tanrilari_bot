...
export interface BattleParticipantChoice {
  country_id: string; country_name: string; side_key: BattleSideKey; is_primary: boolean;
}
...
export interface BattleArmyChoice {
...
async function validateParticipantAvailability(
  ...
  for (const [forceType, quantity] of Object.entries(composition)) {
    ...
    if (requested > stock) {
      ...
      throw new GameError(`**${sourceLabel}** için ordulara tahsis edilmemiş ${label} mevcudu ${stock}; istenen ${requested}.`);
    }
  }
}
...
async function validateSourceSettlement(
  ...
  await validateParticipantAvailability(client, battle, participant, composition, source.id, source.name);
  return source;
}
...
async function rebuildDraftSide(client: DbClient, battleId: string, side: BattleSideKey): Promise<void> {
  ...
  const garrison = (await client.query<{ initial_composition: BattleComposition }>(
    "SELECT initial_composition FROM battle_garrison_assignments WHERE battle_id=$1 AND side_key=$2",
    [battleId, side]
  )).rows[0];
  if (garrison) mergeComposition(composition, garrison.initial_composition);
...
async function rebuildParticipantFromArmies(client: DbClient, battleId: string, countryId: string): Promise<void> {
  const assignments = (await client.query<{ initial_composition: BattleComposition }>(
    "SELECT initial_composition FROM battle_army_assignments WHERE battle_id=$1 AND country_id=$2 ORDER BY army_id",
    [battleId, countryId]
  )).rows;
  const composition: BattleComposition = {};
...
async function applyLossesToDocuments(client: DbClient, battleId: string, guildId: string, actorId: string): Promise<CasualtyApplication[]> {
    ...
    for (const forceType of keys) {
      ...
      if (!naval && forceType === "militia" && side.side_key === "B" && battle.defender_settlement_id && side.temporary_militia > 0) {
        ...
        if (temporaryLoss > 0) {
          ...
          populationApplied += await deductPopulationForCasualties(client, battle.defender_settlement_id, temporaryLoss);
        }
      }

      if (!naval && stateRequested > 0 && side.side_key === "B") {
        const garrison = (await client.query<{ settlement_id: string; initial_composition: BattleComposition }>(
          "SELECT settlement_id,initial_composition FROM battle_garrison_assignments WHERE battle_id=$1 AND side_key='B' FOR UPDATE",
          [battleId]
        )).rows[0];
        const garrisonQuantity = Number(garrison?.initial_composition?.[forceType as BattleForceType] ?? 0);
        const participantQuantity = sideParticipants.reduce((sum, participant) => sum + Number(participant.initial_composition?.[forceType as BattleForceType] ?? 0), 0);
        if (garrison && garrisonQuantity > 0) {
          const sourceShares = allocateLossBySource(
            Math.min(stateRequested, garrisonQuantity + participantQuantity),
            garrisonQuantity + participantQuantity,
            [
              { contractId: "__GARRISON__", quantity: garrisonQuantity },
              { contractId: "__PARTICIPANTS__", quantity: participantQuantity }
            ].filter((source) => source.quantity > 0)
          ).mercenaries;
          const requestedGarrisonLoss = sourceShares.find((share) => share.contractId === "__GARRISON__")?.loss ?? 0;
          const participantLoss = sourceShares.find((share) => share.contractId === "__PARTICIPANTS__")?.loss ?? 0;
          const stackRows = (await client.query<{ id: string; quantity: number }>(
            "SELECT id,quantity FROM unit_stacks WHERE settlement_id=$1 AND force_type='GARRISON' AND unit_type=$2 ORDER BY id FOR UPDATE",
            [garrison.settlement_id, forceType]
          )).rows;
          let remainingGarrisonLoss = requestedGarrisonLoss;
          let appliedGarrisonLoss = 0;
          for (const stack of stackRows) {
            if (remainingGarrisonLoss <= 0) break;
            const deducted = Math.min(Number(stack.quantity), remainingGarrisonLoss);
            const next = Number(stack.quantity) - deducted;
            if (next === 0) await client.query("DELETE FROM unit_stacks WHERE id=$1", [stack.id]);
            else await client.query("UPDATE unit_stacks SET quantity=$1 WHERE id=$2", [next, stack.id]);
            appliedGarrisonLoss += deducted;
            remainingGarrisonLoss -= deducted;
          }
          if (appliedGarrisonLoss > 0) {
            stateApplied += appliedGarrisonLoss;
            populationApplied += await deductPopulationForCasualties(client, garrison.settlement_id, appliedGarrisonLoss);
            garrisonLossSettlements.add(garrison.settlement_id);
          }
          stateRequested = participantLoss;
        }
      }
      ...
      for (const share of participantShares) {
        ...
        if (armyAssignments.length) {
          const armySources = armyAssignments.map((assignment) => ({
            contractId: assignment.army_id,
            quantity: Number(assignment.initial_composition?.[forceType as BattleForceType] ?? 0)
          })).filter((source) => source.quantity > 0);
          const armyTotal = armySources.reduce((sum, source) => sum + source.quantity, 0);
          const armyShares = armyTotal > 0
          ...
          for (const armyShare of armyShares) {
            ...
            const allocationTotal = allocations.reduce((sum, row) => sum + Number(row.quantity), 0);
            const sourceShares = allocationTotal > 0
              ? allocateLossBySource(Math.min(armyShare.loss, allocationTotal), allocationTotal,
                  allocations.map((row) => ({ contractId: row.settlement_id, quantity: Number(row.quantity) }))).mercenaries
              : [];
            for (const sourceShare of sourceShares) {
              const allocation = allocations.find((row) => row.settlement_id === sourceShare.contractId);
              ...
              await client.query("UPDATE armies SET updated_at=NOW() WHERE id=$1", [armyShare.contractId]);
              populationApplied += await deductPopulationForCasualties(client, allocation.settlement_id, sourceApplied);
              stateApplied += sourceApplied;
            }
          }
          continue;
        ...
        const rows = naval
          ...
          : selectedSourceId
            ...
            ? (await client.query<{ id: string; quantity: number; settlement_id: string; force_type: "ARMY" | "GARRISON" }>(
                `SELECT u.id,u.quantity,u.settlement_id,u.force_type FROM unit_stacks u JOIN settlements s ON s.id=u.settlement_id
                  WHERE s.country_id=$1 AND u.unit_type=$2 AND u.force_type='ARMY' AND ($3::uuid IS NULL OR TRUE)
                  ORDER BY CASE u.status WHEN 'FIELD_HOSTILE' THEN 0 WHEN 'FIELD_FRIENDLY' THEN 1 ELSE 2 END,u.id FOR UPDATE OF u`,
        ...
        const deductibleRows = rows.map((row) => {
          ...
          if (naval || (row as {force_type?:string}).force_type==="ARMY") {
            ...
            allocatedBySettlement.set(row.settlement_id, reserved - protectedHere);
          }
          return { ...row, available_quantity: availableQuantity };
        }).filter((row) => row.available_quantity > 0);
        const availableTotal = deductibleRows.reduce((sum, row) => sum + row.available_quantity, 0);
        const rowShares = availableTotal > 0
          ? allocateLossBySource(
              Math.min(share.loss, availableTotal), availableTotal,
              deductibleRows.map((row) => ({ contractId: row.id, quantity: row.available_quantity }))
            ).mercenaries
          : [];
        for (const rowShare of rowShares) {
          const row = deductibleRows.find((candidate) => candidate.id === rowShare.contractId);
          if (!row) continue;
          ...
          if (deducted > 0) {
            const personnelLoss=naval
              ...
              : deducted;
            const populationLoss=await deductPopulationForCasualties(client,row.settlement_id,personnelLoss);
  ...
  await client.query(
    ...
    [guildId, actorId, battleId, JSON.stringify({ automatic: true, coalitionAware: true, persistentArmies: true })]
  );
...
export const battleService = {
  ...
  async create(input: { guildId: string; channelId: string; actorId: string; countryAName: string; countryBName: string; terrain: BattleTerrain; narrative: string; controllerA: BattleController; controllerB: BattleController; defenderSettlementName?: string | null; encounterId?: string | null }): Promise<BattleView> {
    return withTransaction(async (client) => {
      ...
      if (defenderSettlementId) {
        ...
        await client.query(
          "INSERT INTO battle_garrison_assignments(battle_id,side_key,country_id,settlement_id,initial_composition) VALUES($1,'B',$2,$3,$4::jsonb)",
  ...
  async listParticipantArmies(input: { guildId: string; channelId: string; countryName: string }): Promise<BattleArmyChoice[]> {
    const battle = (await pool.query<BattleRow>(
  ...
  async setArmyAssignment(input: { guildId: string; channelId: string; actorId: string; side: BattleSideKey; armyId: string; action: "ADD" | "REMOVE" }): Promise<{ view: BattleView; armyName: string; countryName: string; total: number }> {
    return withTransaction(async (client) => {
      ...
      if (battle.terrain === "NAVAL") throw new GameError("Kalıcı kara orduları deniz savaşı kadrosuna eklenemez.");
      const army = (await client.query<{ id: string; name: string; country_id: string; country_name: string }>(
        `SELECT a.id,a.name,a.country_id,c.name AS country_name FROM armies a JOIN countries c ON c.id=a.country_id
          JOIN battle_side_participants bsp ON bsp.country_id=a.country_id
         WHERE a.id=$1 AND a.guild_id=$2 AND bsp.battle_id=$3 AND bsp.side_key=$4 FOR UPDATE OF a`,
      ...
      if (input.action === "REMOVE") {
        ...
        await client.query("INSERT INTO audit_logs(guild_id,actor_user_id,action,entity_type,entity_id,details) VALUES($1,$2,'battle.army.remove','battle',$3,$4::jsonb)", [input.guildId,input.actorId,battle.id,JSON.stringify({ side: input.side, armyId: army.id, armyName: army.name })]);
        return { view, armyName: army.name, countryName: army.country_name, total: 0 };
      }
      const inOtherBattle = await client.query(
        `SELECT 1 FROM battle_army_assignments baa JOIN battles b ON b.id=baa.battle_id
          WHERE baa.army_id=$1 AND b.id<>$2 AND b.status NOT IN ('FINISHED','CANCELLED') LIMIT 1`, [army.id,battle.id]
      );
      ...
      const composition = Object.fromEntries((await client.query<{ unit_type: string; quantity: number }>(
        "SELECT unit_type,COALESCE(SUM(quantity),0)::integer AS quantity FROM army_units WHERE army_id=$1 GROUP BY unit_type HAVING SUM(quantity)>0",
        [army.id]
      )).rows.map((row) => [row.unit_type,Number(row.quantity)])) as BattleComposition;
      const total = compositionTotal(composition);
      ...
      await rebuildDraftSide(client,battle.id,input.side);
      const view = await loadView(client,battle.id);
      await client.query("INSERT INTO audit_logs(guild_id,actor_user_id,action,entity_type,entity_id,details) VALUES($1,$2,'battle.army.add','battle',$3,$4::jsonb)", [input.guildId,input.actorId,battle.id,JSON.stringify({ side: input.side, armyId: army.id, armyName: army.name, total })]);
      return { view, armyName: army.name, countryName: army.country_name, total };
    });
  },
  ...
  async setMercenaryAssignment(input: { guildId: string; channelId: string; actorId: string; side: BattleSideKey; countryName: string; companyKey: string; action: "ADD" | "REMOVE" }): Promise<BattleView> {
    return withTransaction(async (client) => {
      ...
      if (input.action === "REMOVE") {
        const assignment = (await client.query<{ contract_id: string; initial_land: BattleComposition; initial_ships: BattleComposition; initial_assets: SiegeComposition }>(
          `SELECT bma.contract_id,bma.initial_land,bma.initial_ships,bma.initial_assets FROM battle_mercenary_assignments bma
            JOIN mercenary_contracts mc ON mc.id=bma.contract_id WHERE bma.battle_id=$1 AND bma.side_key=$2 AND mc.company_key=$3 AND mc.country_id=$4 FOR UPDATE OF bma`,
          [battle.id, input.side, input.companyKey, participant.country_id]
        )).rows[0];
      ...
      const contract = (await client.query<{ id: string; country_id: string; status: string }>("SELECT id,country_id,status FROM mercenary_contracts WHERE guild_id=$1 AND company_key=$2 AND country_id=$3 AND status IN ('PENDING','ACTIVE','UNPAID') ORDER BY created_at DESC LIMIT 1 FOR UPDATE", [input.guildId, input.companyKey, participant.country_id])).rows[0];
      if (!contract || !target.country_ids.includes(contract.country_id)) throw new GameError("Bu şirket seçilen ülkeye bağlı değil.");
  ...
  async roll(input: { guildId: string; channelId: string; battleId: string; actorId: string; isGameMaster: boolean }): Promise<{ view: BattleView; side: BattleSideKey; isProxy: boolean }> {
    return withTransaction(async (client) => {
      ...
      const commanderSkill = Number((await client.query<{ skill_bonus: number }>(
        `SELECT COALESCE(MAX(cc.skill_bonus),0)::integer AS skill_bonus
          ...
          WHERE cc.role='COMMANDER'
            AND (
              (cc.assignment='CURIA' AND cc.country_id IN (
      ...
      const formableKeys = (await client.query<{ active_formable_key: FormableCountryKey | null }>(
        ...
        [active.id, side]
      )).rows.map((row) => row.active_formable_key);
      ...
      const longbowDamageBonus = engagedLongbows > 0 && longbowDamageRate > 0
        ...
        : 0;
      if (longbowDamageBonus > 0) {
        roll.damage += longbowDamageBonus;
        roll.detail.briton_longbow = {
          ...roll.detail.briton_longbow!,
          damage: Number(roll.detail.briton_longbow!.damage ?? 0) + longbowDamageBonus
        };
      }
