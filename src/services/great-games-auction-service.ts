  ...
  async lots(guildId: string, countryId?: string): Promise<AuctionLot[]> {
    return (await pool.query<AuctionLot>(
      `SELECT l.id,l.title,l.reward_type,l.lot_order,l.phase,l.winning_country_id,c.name AS winning_country_name,l.winning_bid,
         ...
         LEFT JOIN LATERAL (
           SELECT b.amount,c2.name AS country_name
           FROM great_games_auction_bids b
           JOIN countries c2 ON c2.id=b.country_id
           WHERE b.lot_id=l.id
  ...
  async bid(input: { guildId: string; countryId: string; userId: string; lotId: string; amount: number }): Promise<{ phase: string; reserved: number }> {
    return withTransaction(async (client) => {
      ...
      const highest = Number((await client.query<{ amount: number }>(
        "SELECT COALESCE(MAX(amount),0)::bigint AS amount FROM great_games_auction_bids WHERE lot_id=$1", [lot.id]
      )).rows[0]?.amount ?? 0);
      const minimum = auctionNextMinimum(highest);
    ...
    return withTransaction(async (client) => {
      ...
      for (const lot of lots) {
        const bids = (await client.query<{
          id: string; country_id: string; country_name: string; amount: number; reserved_amount: number;
        }>(
        ...
        for (const bid of bids.slice(1)) {
          refunded += await refundBid(client, active.id, bid, "kalemi kazanamadı");
        }
        if (!winner) {
          await client.query("UPDATE great_games_auction_lots SET phase='FINISHED' WHERE id=$1", [lot.id]);
        ...
        await client.query(
          ...
          [winner.country_id, winner.amount, lot.id]
        );
      ...
      await client.query(
        "UPDATE great_games_entries SET status='FINISHED' WHERE season_id=$1 AND game_type='AUCTION' AND status='ACTIVE'",
        [active.id]
      );
