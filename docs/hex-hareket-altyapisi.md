...
4. Devletin etkin yerel üretiminden veya tamamlanmış ticaretinden **At** erişimi varsa kara orduları temel hareket hakkının %25'i kadar hız kazanır; kesir yukarı yuvarlanır. Örneğin temel hakkı 4 Hex olan ordu +1, 5 Hex olan ordu +2 Hex kazanır. Birden fazla At erişimi bonusu katlamaz. Kendi hammaddesinin tamamını ticarete ayırmış yerleşke o kaynağı devlete ayrıca sağlamaz. Bonus normal ordu hareketinde ve farklı yerleşkelerden orduya yürüyen asker intikalinde uygulanır. Şu anda filolara hız veren tanımlı bir hammadde yoktur.
5. Şimdilik yalnızca normal hareket emri kabul edilir. Stratejik/zorunlu/gizli yürüyüş kipleri veri modelinde yer alır ancak yorgunluk ve istihbarat sonuçları tamamlanana kadar kapalıdır.
6. Tur çözümleyici kayıtlı emrin o tura sığan adımlarını ilerletir. Başka devletin kara bölgesine giriş, düşman birliğiyle temas ve farklı devletlerin eşzamanlı rota kesişmesi otomatik savaş başlatmaz; emir **BLOCKED** olup tek bir yönetici dosyasına bağlanır. Kayıtlı gözcü/keşif temasları gizli istihbarat kontrolü üretir.
7. Etkin emri bulunan birliklerin kadrosu/komutanı ve elle konumu değiştirilemez. Aynı birlik bir turda tamamlanan emrin ardından ikinci emirle ilave hareket kazanamaz.
...
Kalıcı ordular klasik tahsis modelini kullanır. Askerin toplam mevcudu yerleşkenin askerî stokunda tutulur; `/ordu asker-ekle` yalnız seçilen miktarı orduya ayırır. `/ordu asker-cikar` ve `/ordu dagit` tahsisi kaldırır, stok miktarını ikinci kez artırmaz. Böylece ordudan çıkarılan asker aynı yerleşkeden başka bir orduya doğrudan tahsis edilebilir.

Koordinatlı hareket sistemi bu geri dönüşte duraklatılmıştır. Ordu kadrosu; Hex, hareket emri, karşılaşma veya gemiye binme kaydı nedeniyle kilitlenmez. Ordu toplama intikali oluşturulmaz; farklı yerleşkelerden seçilen müsait askerler eski düzende anında ordu tahsisine girer.
